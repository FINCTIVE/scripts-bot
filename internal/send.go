...
func SendQuick(bot *tb.Bot, to tb.Recipient, message string, options ...interface{}) (tgMessage *tb.Message) {
	return Send(bot, to, message, "", "", options...)
}
...
func Send(bot *tb.Bot, to tb.Recipient,
	message, prefix, suffix string, options ...interface{}) (tgMessage *tb.Message) {
	if len(message) == 0 {
		return nil
	}
	...
	for {
		var err error
		tgMessage, err = bot.Send(to, prefix+messages[0]+suffix, options...)
		if err != nil {
			...
			if retryCounter >= maxSendRetry {
				...
				_, lastErr := bot.Send(to, lastSignalMessage, options...)
				if lastErr != nil {
...
func SendUpdate(bot *tb.Bot, to tb.Recipient, messageBytes *[]byte, prefix, suffix string,
		...
		for {
			...
			case <-time.After(time.Second):
				...
				if msg == nil {
					...
					msg, err = bot.Send(to, messageStr, options...)
					if err != nil {
					...
					if msg.Text != messageStr {
						...
						_, _ = bot.Edit(msg, messageStr, options...)
					}
				}
			case <-ctx.Done():
				if msg != nil {
					deleteErr := bot.Delete(msg)
					if deleteErr != nil {
